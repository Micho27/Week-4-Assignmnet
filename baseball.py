...
More details about expected output can be found
onthe coursera site for "Python Data Analysis"
"""
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    for row in statistics:
        ...
        # create the tuple and add to list
        temp_tuple = tuple((player_id, comp_stat))
        my_list.append(temp_tuple)
    
    # sort the list by descending stat
    my_list.sort(key = lambda pair: pair[1], reverse=True)
    # limit the amount of items in the list to numplayers
    for tuple_idx in my_list:
        if numplayers > 0:
            short_list.append(tuple(tuple_idx))
            numplayers -= 1
        else: break
    return short_list
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    my_list = []
    # read master file as nested dict
    info_dict = read_csv_as_nested_dict(info['masterfile'], info['playerid'], info['separator'], info['quote'])
    
    for pair in top_ids_and_stats:
        format_name = format(pair[1], '.3f') + " --- {0} {1}".format(info_dict[pair[0]][info['firstname']],
                                                                         info_dict[pair[0]][info['lastname']])
        my_list.append(format_name)
    return my_list
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for row in statistics:
        #first time you see a player, create a new entry in agg_dict
        if row[playerid] not in agg_dict:
            ...
            agg_dict[row[playerid]] = dict(stats_dict)
        #if the player is already in agg_dict
        else:
            for field in fields:
                agg_dict[row[playerid]][field] += int(row[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    stats_list = []
    agg_stats = aggregate_by_player_id(read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote']),
                                              info['playerid'],
                                              info['battingfields'])
    
    #convert nested dict to list of dictionaries
    for dictionary in agg_stats.values():
        stats_list.append(dictionary)
